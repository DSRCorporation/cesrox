...
use crate::primitives::prefix::SelfAddressingPrefix;
...
pub enum SelfAddressingCode {
    Blake3_256,
    Blake2B256 { key: Vec<u8> },
    Blake2S256 { key: Vec<u8> },
    SHA3_256,
    SHA2_256,
    Blake3_512,
    SHA3_512,
...
impl DerivationCode for SelfAddressingCode {
...
impl FromStr for SelfAddressingCode {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
            ...
            .ok_or_else(|| CesrError::DeserializeError("Empty prefix".into()))?
        {
            ...
            "F" => Ok(Self::Blake2B256 { key: vec![] }),
            "G" => Ok(Self::Blake2S256 { key: vec![] }),
            "H" => Ok(Self::SHA3_256),
            "I" => Ok(Self::SHA2_256),
            "0" => match &s[1..2] {
...
mod self_addressing_tests {
    ...
    fn test_self_addressing() {
        let der = SelfAddressingCode::Blake3_256.derive(b"abcdefghijklmnopqrstuvwxyz0123456789");
        assert_eq!(der.to_str(), "EsLkveIFUPvt38xhtgYYJRCCpAGO7WjjHVR37Pawv67E");

        let der = SelfAddressingCode::Blake3_512.derive(b"abcdefghijklmnopqrstuvwxyz0123456789");
        assert_eq!(der.to_str(), "0DsLkveIFUPvt38xhtgYYJRCCpAGO7WjjHVR37Pawv67GaNK7TEsvL8TnpL3E8EF7fR4KFgQaeMYFMM0mFNWmP-g");

        let der = SelfAddressingCode::SHA2_256.derive(b"abcdefghijklmnopqrstuvwxyz0123456789");
        assert_eq!(der.to_str(), "IAR_CmU450lEUFUD4emkJKz8iqGdn9yg95-7ts4l77fY");

        let der = SelfAddressingCode::SHA2_512.derive(b"abcdefghijklmnopqrstuvwxyz0123456789");
        assert_eq!(der.to_str(), "0GpZtJIWoOOiBEO3LGS9rlHUGzOtCKhqT7k2N43S-c04mYCewx5SWbO0VJOI0CZWE2K-cVSNQ5O-dtp-6wGDlHDA");

        let der = SelfAddressingCode::Blake2B512.derive(b"abcdefghijklmnopqrstuvwxyz0123456789");
        assert_eq!(der.to_str(), "0FefJm_XA2SP8c_tg8TZDS0tnho1iFCO15rIXDksOBFqkEpPFOu9r8hoEjBaFP_ewvUnZoOegECZ1_qEF5h9bcJQ");

        let der = SelfAddressingCode::SHA3_256.derive(b"abcdefghijklmnopqrstuvwxyz0123456789");
        assert_eq!(der.to_str(), "HAV1P0Jucuk5IyFE_LxP955z5dh52NpDhXmEnrqDJ8cU");

        let der = SelfAddressingCode::SHA3_512.derive(b"abcdefghijklmnopqrstuvwxyz0123456789");
