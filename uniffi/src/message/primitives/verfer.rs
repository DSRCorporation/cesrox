...
pub use cesrox::{Verfer, Matter};
use crate::message::primitives::MatterCodex;

pub fn verfer_new_with_code_and_raw(code: &MatterCodex, raw: &[u8]) -> ParsideResult<Verfer> {
    Verfer::new_with_code_and_raw(code.code(), raw).map_err(ParsideError::from)
}
