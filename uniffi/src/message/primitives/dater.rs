...
pub use cesrox::{Dater, Matter};
use crate::message::primitives::MatterCodex;

pub fn dater_new_with_code_and_raw(code: &MatterCodex, raw: &[u8]) -> ParsideResult<Dater> {
    Dater::new_with_code_and_raw(code.code(), raw).map_err(ParsideError::from)
}
